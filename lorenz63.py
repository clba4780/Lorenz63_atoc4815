...
class Lorenz63:
    ...
    def run_ensemble(self, initial_conditions, dt, n_steps):
        ...
        #define n_member with the initial conditions
        """n_members = initial_conditions.shape[0]
        ensemble = np.zeros((n_members, n_steps+1, 3))

        for i in range(n_members):
            ensemble[i] = self.run(initial_conditions[i], dt, n_steps)
        return ensemble
      """
     #define the size of n_members
        n_members = initial_conditions.shape[0]
        #create an ensemble to store each value using np.zeros
        ensemble = np.zeros((n_members, n_steps+1, 3))
        #define state
        ...
        for t in range(n_steps):
            states = states + self.vectorized_tendency(states)*dt
            ensemble[:,t+1,:] = states

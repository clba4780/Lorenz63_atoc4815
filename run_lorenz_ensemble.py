...
def main():
    ...
    spinup_traj = model.run(state0=np.array([1,1,1]), dt=DT, n_steps = SPINUP_STEPS)

    final_state = spinup_traj[-1]

    ref_trajectory = model.run(state0=final_state, dt=DT, n_steps = REFERENCE_STEPS)
    ...
    # )
    plot_ensemble_panels(
          [ensemble_deep, ensemble_high, ensemble_saddle], ref_trajectory, 
          ["(a)Deep Left Lobe", "(b)High Left Lobe", "(c)Saddle Region"],
          save_path = SAVE_PATH)
...
# Notes: by red trajectories are much shorter than i would expect, by title for each panel is not appearing
